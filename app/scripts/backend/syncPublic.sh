...
dest_dir=/var/www/html/clinomics_public
python_env=/var/www/html/clinomics.env
sample_list=""

while [[ "$#" -gt 0 ]]; do case $1 in
  -p|--prj) projects="$2"; shift;;
...
if [ -z $projects ]; then
	...
	echo
	echo "required:"
	echo "-p: project name list (comma seperated)"
	echo 
	echo "optional:"
...
source_dir=/var/www/html/clinomics
data_dir=/var/www/html/clinomics/app/storage/data

public_cases_file=${projects}_cases.tsv
...
if [[ $patient_mapping_file ]]; then
	...
	perl $source_dir/app/scripts/backend/getProjectCases.pl -p $projects -m $patient_mapping_file > $public_cases_file
else
	echo "$source_dir/app/scripts/backend/getProjectCases.pl -p $projects > $public_cases_file"
	perl $source_dir/app/scripts/backend/getProjectCases.pl -p $projects > $public_cases_file
fi
...
if [ $do_master = "1" ]; then
	...
	echo "python $dest_dir/app/scripts/backend/extractFromMaster.py $khanlab_master_source_file $projects $khanlab_master_file $sample_list"
	python $dest_dir/app/scripts/backend/extractFromMaster.py $khanlab_master_source_file "$projects" $khanlab_master_file $sample_list
	python $dest_dir/app/scripts/backend/extractFromMaster.py $outside_master_source_file "$projects" $outside_master_file $sample_list
	python $dest_dir/app/scripts/backend/extractFromMaster.py $clinomics_master_source_file "$projects" $clinomics_master_file $sample_list

	khanlab_modified="1"
	echo "Uploading public database..."
	if [[ $patient_mapping_file ]]; then
		perl ${dest_dir}/app/scripts/backend/syncMaster.pl -u -n public -i $khanlab_master_file,$outside_master_file,$clinomics_master_file -m 1,1,1 -p $patient_mapping_file
	else
		echo "$khanlab_master_file,$outside_master_file,$clinomics_master_file"
		perl ${dest_dir}/app/scripts/backend/syncMaster.pl -u -n public -i $khanlab_master_file,$outside_master_file,$clinomics_master_file -m 1,1,1
	fi
	echo "all done!";
fi
...
if [ $do_load = "1" ] || [ $do_tier = "1" ] || [ $do_fusion = "1" ]; then
	...
	do
		...
		if [ $do_load = "1" ]; then
			...
			perl $dest_dir/app/scripts/backend/loadVarPatients.pl -i $public_data_home -u $url -p $dest_patient_id -c $dest_case_id -y
		fi
		if [ $do_tier = "1" ]; then
			echo "Loading AVIA Tiering($i/$total_cases) : $path/$src_patient_id/$src_case_id => $path/$dest_patient_id/$dest_case_id"
			perl $dest_dir/app/scripts/backend/loadVarPatients.pl -i $public_data_home -u $url -p $dest_patient_id -c $dest_case_id -t tier
		fi
		if [ $do_fusion = "1" ]; then
			echo "Loading Fusion($i/$total_cases) : $path/$src_patient_id/$src_case_id => $path/$dest_patient_id/$dest_case_id"
			perl $dest_dir/app/scripts/backend/loadVarPatients.pl -i $public_data_home -u $url -p $dest_patient_id -c $dest_case_id -t fusion
		fi
		i=$((i+1))
	done < $public_cases_file
	perl $dest_dir/app/scripts/backend/updateVarCases.pl
fi
...
#./runSqlldr.sh connect_string ../../storage/data/var_tier_avia.tsv ctrl_files/var_tier_avia.ctrl ../../storage/data/var_tier_avia.bad ../../storage/data/var_tier_avia.log
#
